...
if ( ! class_exists( 'Magazine_Shop_About' ) ) {
	...
	class Magazine_Shop_About {
		 ...
		 * Single instance.
		 *
		 * @var Magazine_Shop_About $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Magazine_Shop_About ) ) {
				self::$instance = new Magazine_Shop_About;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( 'About %s', 'magazine-shop' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( 'About %s', 'magazine-shop' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-about';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'magazine-shop' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'magazine-shop' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-hello="world" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'magazine_shop_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'magazine-shop' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_magazine_shop_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_magazine_shop_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_page() {
			?>
			<div class="wrap about-wrap twp-about-wrap">
				...
				<?php endif; ?>
				<a href="https://themeinwp.com" target="_blank"><div class="wp-badge"></div></a>
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'magazine-shop' ), $method );
			}
		}
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section twp-section twp-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .twp-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section twp-section twp-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .twp-section -->
		...
		public function useful_plugins() {
			...
			?>
			<div class="twp-section twp-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'magazine-shop' ); ?></a>
				<?php endif; ?>
		...
		public function demo_content() {
			...
			?>
			<div class="twp-section twp-section-demo-content">
		...
		public function upgrade_to_pro() {
			...
			?>
			<div class="twp-section twp-section-upgrade-to-pro">
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'magazine-shop-about', get_template_directory_uri() . '/inc/about/css/about' . $min . '.css', array(), '1.0.0' );
				wp_enqueue_script( 'magazine-shop-about', get_template_directory_uri() . '/inc/about/js/about' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'magazine_shop_dismiss_status', true );
			?>
			<?php if ( current_user_can( 'edit_theme_options' ) && 'themes' === $screen_id && 1 !== absint( $dismiss_status ) ) : ?>
				<div class="twp-about-notice notice notice-about">
					<?php echo $this->notice; ?>
				</div><!-- .twp-about-notice -->
			<?php endif; ?>
			<?php
		}
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'magazine_shop_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'magazine_shop_dismiss_status', 1 );
