...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content">
		...
		<?php if ( ! is_single() ){ // check if it is single page
			if( empty( $video ) ){ ?>
				<figure>
		 ...
		 } else{ // for detail page video section
				 ?>
			<figure>
				<?php if(has_post_thumbnail()){ ?>
				...
				<div class="bazz_single_content row">
					<header class="bazz_single_meta entry-header col-lg-3 col-md-12 bazz_single_content__1">
						<div class="entry-meta">
						<?php the_category(); ?>
						<?php if( has_tag() ){ ?>
							<div class="blog-cat">
					...
					<div class="blog_excerpt col-lg-9 col-md-12">
						<?php the_content(); ?>
					</div>
				</div>
		<?php }
